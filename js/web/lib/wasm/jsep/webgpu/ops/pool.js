...
import {PoolConvUtil, ShapeUtil} from '../../util.js';
import {/*AttributeWithCacheKey,*/ createAttributeWithCacheKey} from '../attribute-with-cache-key.js';
import {/*ComputeContext,*/ GpuDataType/*, ProgramInfo, ProgramMetadata*/} from '../types.js';
 ...
 * @template {AveragePoolAttributes|MaxPoolAttributes} AttributeType
 * @returns {[AttributeType, number[]]}
 */
const getAdjustedPoolAttributesAndOutputShape = (inputs, attributes, isGlobalOperator) => {
  ...
  /** @type {number[]} */
  const dilations = hasDilations ? (attributes /*as MaxPoolAttributes*/).dilations.slice() : [];
 ...
 * @param {boolean} isGlobalOperator
 * @param {AveragePoolAttributes} attributes
 * @returns {ProgramInfo}
 */
 ...
 *
 * @param {Record<string, unknown>} attributes
 * @returns {AveragePoolAttributes}
 */
export const parseAveragePoolAttributes = (attributes) => {
 ...
 * @param {AveragePoolAttributes} attributes
 */
export const averagePool = (context, attributes) => {
 ...
 * @returns {AveragePoolAttributes}
 */
export const parseGlobalAveragePoolAttributes = (attributes) => {
 ...
 * @param {AveragePoolAttributes} attributes
 */
export const globalAveragePool = (context, attributes) => {
  validateInputs(context.inputs);
 ...
 * @param {ProgramMetadata} metadata
 * @param {boolean} isGlobalOperator
 * @param {MaxPoolAttributes} attributes
 * @returns {ProgramInfo}
 */
 ...
 * @param {MaxPoolAttributes} attributes
 */
export const maxPool = (context, attributes) => {
  validateInputs(context.inputs);
 ...
 *
 * @param {Record<string, unknown>} attributes
 * @returns {MaxPoolAttributes}
 */
export const parseMaxPoolAttributes = (attributes) => {
 ...
 * @returns {MaxPoolAttributes}
 */
export const parseGlobalMaxPoolAttributes = (attributes) => {
  const format = attributes.format /*as FormatAttributes['format']*/;
  return {format, ...globalPoolAttributes, cacheKey: format};
};
/**
 ...
 * @param {MaxPoolAttributes} attributes
 */
export const globalMaxPool = (context, attributes) => {
  validateInputs(context.inputs);
